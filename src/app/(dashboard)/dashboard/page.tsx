    ...
    <div className="space-y-10 pb-12">
      <div className="border-b border-zinc-800 pb-5">
        <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2 text-zinc-50">
          <LayoutDashboard className="h-8 w-8 text-[#192e5b]" /> Dashboard
        </h1>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold tracking-wider text-zinc-50 uppercase">Organização & Editais</h2>
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="border-zinc-800 bg-zinc-900 text-zinc-50">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Compass className="h-4 w-4 text-[#ff5f3a]" />
            </CardHeader>
            <CardContent>
              {loadingStats ? <Loader2 className="h-7 w-7 animate-spin text-zinc-600" /> : <div className="text-3xl font-bold tracking-tight text-[#ff5f3a]">{totalRoutes}</div>}
            </CardContent>
          </Card>
          ...
          <Card className="border-zinc-800 bg-zinc-900 text-zinc-50">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <BookOpen className="h-4 w-4 text-[#192e5b]" />
            </CardHeader>
            <CardContent>
              {loadingStats ? <Loader2 className="h-7 w-7 animate-spin text-zinc-600" /> : <div className="text-3xl font-bold tracking-tight text-[#192e5b]">{totalSubjects}</div>}
            </CardContent>
          </Card>
          ...
          <Card className="border-zinc-800 bg-zinc-900 text-zinc-50">
            ...
            <CardContent>
              {loadingStats ? <Loader2 className="h-7 w-7 animate-spin text-zinc-600" /> : <div className="text-3xl font-bold tracking-tight text-emerald-500">{completionPercentage}%</div>}
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold tracking-wider text-zinc-50 uppercase">Desempenho Geral (Questões)</h2>
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="border-zinc-800 bg-zinc-900 text-zinc-50">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-zinc-400">Total Resolvidas</CardTitle>
              <BarChart3 className="h-4 w-4 text-[#ff5f3a]" />
            </CardHeader>
            <CardContent>
              {loadingStats ? <Loader2 className="h-7 w-7 animate-spin text-zinc-600" /> : <div className="text-3xl font-bold tracking-tight text-[#ff5f3a]">{globalQuestions}</div>}
            </CardContent>
          </Card>
          ...
          <Card className="border-zinc-800 bg-zinc-900 text-zinc-50">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Award className="h-4 w-4 text-[#192e5b]" />
            </CardHeader>
            <CardContent>
              {loadingStats ? <Loader2 className="h-7 w-7 animate-spin text-zinc-600" /> : <div className="text-3xl font-bold tracking-tight text-[#192e5b]">{globalRate}%</div>}
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold tracking-wider text-zinc-50 uppercase">Gráfico de Linha de Tendência</h2>
        <Card className="border-zinc-800 bg-zinc-900 text-zinc-50">
          <CardHeader>
            <CardTitle className="text-base font-medium">Evolução do Aproveitamento</CardTitle>
            <CardDescription className="text-zinc-400 text-xs">Acompanhe seu percentual de acertos em ordem cronológica.</CardDescription>
          </CardHeader>
          <CardContent className="pt-2">
            {loadingStats ? (
              ...
              <div className="h-[240px] flex items-center justify-center text-zinc-500 text-sm italic">
                Insira registros de questões para gerar a tendência.
              </div>
            ) : (
                  ...
                  <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="colorRate" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#192e5b" stopOpacity={0.2}/>
                        <stop offset="95%" stopColor="#192e5b" stopOpacity={0}/>
                      </linearGradient>
                    </defs>
                    ...
                    <Tooltip 
                      contentStyle={{ backgroundColor: '#18181b', borderColor: '#192e5b', borderRadius: '8px', color: '#f4f4f5', fontSize: '12px' }}
                      itemStyle={{ color: '#ff5f3a' }}
                    />
                    <Area type="monotone" dataKey="Aproveitamento (%)" stroke="#ff5f3a" strokeWidth={2} fillOpacity={1} fill="url(#colorRate)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            )}
